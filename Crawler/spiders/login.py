...
class LoginSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if len(captcha) > 0:
            print("此时有验证码")
            # 设置将验证码图片存储到本地的本地地址
            localpath = "c:\\douban\\captcha.png"
            ...
            captcha_value = input()
            # 设置要传递的post信息
            data = {# 设置登录账号，格式为账号字段名:具体账号
                "form_email": "13816718408", # 设置登录密码，格式为密码字段名:具体密码，读者需要将账号密码换成自己的
                # 因为笔者完成该项目后已经修改密码
                "form_password": "13819392572", # 设置验证码，格式为验证码字段名:具体验证码
                "captcha-solution": captcha_value, # 设置需要转向的网址，由于我们需要爬取个人中心页，所以转向个人中心页
                "redir": "https://www.douban.com/people/yixinggee/", }
        # 否则说明captcha列表中没有元素，即此时不需要输入验证码信息
        else:
            print("此时没有验证码")
            # 设置要传递的post信息，此时没有验证码字段
            data = {"form_email": "13816718408", "form_password": "13819392572",
                "redir": "https://www.douban.com/people/yixinggee/", }
        print("登录中…")
        # 通过FormRequest.from_response()进行登录
        return [FormRequest.from_response(response,  # 设置cookie信息
                                          meta={"cookiejar": response.meta["cookiejar"]},  # 设置headers信息模拟成浏览器
                                          headers=self.header,  # 设置post表单中的数据
                                          formdata=data,  # 设置回调函数，此时回调函数为next()
                                          callback=self.next, )]
    ...
    def next(self, response):
        ...
        datas = str(response.body)
        notetitles =response.xpath('//div[@class="bd sns"]')
        for titles in notetitles:

            notetitle = titles.xpath('./div[@class="block note-block"]/div/div/a/text()').extract()
            notetime = response.xpath('./div[@class="actions"]/span/a/text()').extract()
            notecontent = response.xpath('./div[@class="block note-block"]/div/p/text()').extract()
            noteurl = response.xpath('./div[@class="block note-block"]/div/div/a/@href').extract()
            print("网页标题是：" + title[0])
            # 可能有多篇日记，通过for循环依次遍历
